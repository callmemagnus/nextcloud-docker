...
mode="${NEXTCLOUD_DOCKER_MODE:-apache}"
if expr "$1" : "apache" 1>/dev/null; then
    mode="apache"
elif expr "$1" : "php-fpm" 1>/dev/null; then
    mode="php-fpm"
fi

if test "$mode" != "apache" && test "$mode" != "php-fpm"; then
    echo Looks like you provided a bad value for NEXTCLOUD_DOCKER_MODE.
    echo Allowed values are \"apache\" or \"php-fpm\".
    echo "You provided NEXTCLOUD_DOCKER_MODE=\"$mode\", cannot continue."
    exit 1
fi


if test "$mode" == "apache"; then
